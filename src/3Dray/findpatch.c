...
int main(int ac, char **av)
{
    ...
    getpar("dlatshift",  "F",   &dlatshift); /* mapping grid */
    getpar("dlonshift",  "F",   &dlonshift); /* mapping grid */
...
*/
    ...
    if(sach.t2 > -10000 && CalEnergy < 0){
        Tmark    = sach.t2 + PKPt2 - sach.t0;
    }
    ...
    setgrids(dlat, dlon, dlatshift, dlonshift, ndata, sach.gcarc, sout, osf);

    ndata = patch(old, pole, ts2rf, dr2sf, PKPt2, sach, amp, period, checknoise, sout, PKPduration);
    setgrids(dlat, dlon, dlatshift, dlonshift, ndata, sach.gcarc, sout, orf);
...
*/
	...
	while( fabs(lat2 - lat1) > TINY){
	    dist = delta(0.5*(lat2+lat1), lon1, pnew.lat, pnew.lon);
	    t3  = findtime(ntd,td,dist);
	    t1  = findtime(*ntt,tt,0.5*PI-0.5*(lat2+lat1));
/*
	fprintf(stderr, "not enough points j = %d t1=%lf t3=%lf %lf %lf\n",ntt0,t1,t3,lat1/RAD, lat2/RAD);
*/
            if(t3 > 0 && t1 > 0)
	        t3 += t1;
            else  
...
double findtime(int n, TTIME *tt, double dist)
{
	...
	fprintf(stderr, "Warning:: Did not find time for dist=%lf in %lf - %lf\n", dist, tt[0].r, tt[n-1].r);
	return 0.0;
    } else {
	found = 0;
	for(i=0; i<n-1; i++){
